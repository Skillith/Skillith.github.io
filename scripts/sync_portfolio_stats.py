    ...
    metrics_payload = {
        ...
        'apps': {
            ...
            'terracatch': {
                ...
                'monetization_desc': 'Evolution token packs and exhibition battle passes via Google Play Billing.',
            ...
            'pingquest': {
                ...
                'monetization_desc': 'Campaign host pass ($15/mo) via Stripe billing integration.',
                ...
                'ai_cost': '$0.35',
                'ai_desc': 'Gemini 3.7 Flash high-throughput prompt caching per 1k action checks.',
                'insight': f'Room utilization: {pq_rooms} dedicated campaign party rooms actively hosting living chronicles.',
                'funnel': [
            ...
            'guide': {
                ...
                'installs': wg_accounts,
                'installs_label': 'Alpha Roadtrippers & Drivers',
                'active_now': wg_active_now,
                'dau': wg_dau,
                'paying': 0,
                ...
                'core_action_name': 'Driver Profiles',
                'core_action_count': wg_accounts,
                'core_action_desc': 'Dynamic GPS coordinate listeners & automated folklore narration streams.',
                'ai_cost': '$1.85',
                ...
                'funnel': [
                    {'stage': 'Driver Signups & Mobile Installs', 'count': wg_accounts, 'pct': '100%'},
                    {'stage': 'GPS Stream Permission Allowed', 'count': max(wg_accounts - 4, 1), 'pct': f"{round(max(wg_accounts - 4, 1) / max(wg_accounts, 1) * 100, 1)}%"},
                    {'stage': 'First Road-trip Narration Completed', 'count': max(wg_accounts - 12, 1), 'pct': f"{round(max(wg_accounts - 12, 1) / max(wg_accounts, 1) * 100, 1)}%"},
            ...
            'dalil': {
                ...
                'conversion_rate': '0.0%',
                'is_monetized': True,
                'pricing_model': 'Pro Tour License',
                'monetization_desc': 'Unlimited offline tour routes and serverless speech quota upgrades.',
